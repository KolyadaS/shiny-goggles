...
let rangeMin = document.querySelector('.range-slider__block-min');
let rangeMax = document.querySelector('.range-slider__block-max');
console.log(rangeMin);
console.log(rangeMax);


// координаты кружочков слайдера
let rangeMinCoords = rangeMin.getBoundingClientRect();
let rangeMaxCoords = rangeMax.getBoundingClientRect();
console.log(rangeMinCoords);
console.log(rangeMaxCoords);

// вычисление координат при нажатии на кружки слайдера
function onMouseDown() {
    event.preventDefault(); // выключаем стандартный обработчик браузера
    // event.target - это объект, на который щелкнули мышью
    console.log(event.target);
    let coord = this.getCoordinates(event.target);
    console.log(coord);
}
...
// получение координат курсора мыши для двигания кружочка в onMouseUp
function getCursorPosition(){
    console.log('cursorX = ' + event.pageX);
    return event.pageX;
}
...
function onMouseUp() {
    ...
    let coord = this.getCoordinates(event.target);
    console.log(coord);
    let myCursorPosition = getCursorPosition();
    //event.target.style.left = coord.left + 50 + 'px'; //при отпускании мыши кружок перемещался влево на 50рх
    console.log(event.target);
    return event.target.style.left = myCursorPosition + 'px'; // кружок двигается, но рывками
}
